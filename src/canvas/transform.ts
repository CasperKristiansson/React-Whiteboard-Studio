...
export const screenPointToWorld = (point: Vec2, viewport: Transform): Vec2 => ({
  x: point.x / viewport.scale + viewport.x,
  y: point.y / viewport.scale + viewport.y,
})

export const worldPointToScreen = (point: Vec2, viewport: Transform): Vec2 => ({
  x: (point.x - viewport.x) * viewport.scale,
  y: (point.y - viewport.y) * viewport.scale,
})

export const screenDeltaToWorld = (delta: Vec2, viewport: Transform): Vec2 => ({
